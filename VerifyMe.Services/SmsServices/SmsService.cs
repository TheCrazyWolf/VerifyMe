...
public class SmsService(VerifyStorage storage, ITelegramBotClient botClient)
{
    public async Task<SmsResult> SendSmsAsync(App app, string phoneNumber, string message, IReplyMarkup? replyMarkup = null)
    {
        ...
        var user = await storage.Users.GetUserByPhoneAsync(phone: phoneNumber);
        ...
        var isSuccess = await botClient.TrySendMessageAsync(chatId: user.Id, message: message, replyMarkup: replyMarkup);
        Sms sms = new Sms { AppId = app.Id, UserId = user.Id, Message = message, IsDelivered  = isSuccess, DateTimeSend = DateTime.Now};
        await storage.Sms.CreateSmsAsync(sms: sms);
    ...
    {
        IList<SmsResultExtended> resultExtends = new List<SmsResultExtended>();
        foreach (var dto in dtos)
        {
            var result = await SendSmsAsync(app, dto.Phone, dto.Message);
            resultExtends.Add(new SmsResultExtended(result.IsSuccess, dto.Phone, result.SystemMessage));
    ...
    {
        return await storage.Sms.GetSmsByAppIdAsync(appId: appId);
    }
